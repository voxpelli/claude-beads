...
> **Lead motif** _(every disposition in this document defers to this sentence; a
> surface that doesn't resolve to one of the three homes below is a surface still
> pretending to be something it isn't):_
>
...
This is a _concept_ document, not a runbook. It names a misclassification, proposes
...
1. **STATE** — structured, tool-parsed, schema'd, has a lifecycle (open → resolved),
   ...
   a session-start prime that surfaces it _from the tool_ rather than from filesystem
   noise.
...
every operational test they are identical in _kind_ to `.diarie/`:
...
tool to surface them, _filesystem-visibility was the only way to not forget a friction
point._ A caps-locked file at the repo root is impossible to miss in an `ls`; that
...
**compact-branch UPSTREAM summary** are _workarounds for the fact that the state
isn't behind a tool yet_. They exist to shout "you have untriaged UPSTREAM/SYNERGY
...
independent: `diarie` tracks _this project's work_; `ledger` tracks _this project's
relationships to other things_. Two stores, two tools, one shared principle.
...
The blunt question — _rebuild for all, or some?_ — answered precisely, surface by
...
load-bearing: it keeps the file _outside_ the `SYNERGY-*.md` glob, so every public
consumer (retrospective, sprint-review, session-start, promotion, reciprocation)
_structurally cannot_ read it — the privacy is a filesystem fact, not a per-consumer
...
read path filters _and_ a gitignore glob that keeps the file off disk in a clone) —
whichever the ledger design picks, the test is the same one that governs today: a
public consumer must be _unable_ to read a private entry, not merely _choosing_ not
...
`RETRO-*.md` is not rebuilt anywhere — the file _type ceases to exist._ The plan is
...
`session-start.sh` even looks at them, and only to _count_ them). This is the natural
...
* **A sprint-scoped counter** — a small piece of committed state (a single integer,
  ...
  4-sprint cadence.
* **Drop the mechanic** — if trend review is better triggered by _condition_ (backlog
  staleness, unreviewed-friction count) than by a modular _count_, the mod-4 heuristic
...
per CLAUDE.md `### Files-availability convention` (_"require-or-fallback"_): it sources
...
* **When diarie is PRESENT** — run-state can live in **diarie rows** (task rows carry
  ...
  _mirrors_ them (`skills/swarm-wave/SKILL.md` "## SWARM Files").
* **When diarie is ABSENT** (ROADMAP or manual source) — the **`SWARM-NN.md` Item
  ...
  substrates` states this verbatim: _"a trackerless wave uses the `SWARM-NN.md` Item
  Status table as run-state"_, and the SKILL's "## SWARM Files" makes the orchestrator
...
_only_ when there is no tracker to hold it. It is not state pretending to be
...
field on a diarie task can _speed_ the file-disjoint partition when diarie is present
...
standalone skill (`vp-swarm`), not a part of `diarie`. diarie is a task _store_;
swarm-wave is a multi-agent _orchestrator_ that can read that store, or a roadmap, or
...
_all_ substrates: the **file-disjoint partition** (its core safety mechanism — agents
...
* **The safe reading: diarie only STORES `scope`.** The field is data on a row; the
  _partition computation_ (contention map + disjoint-set clustering) stays **skill-side**
  in swarm-wave, over `diarie ready --json` _or_ the trackerless source. This keeps
...
* **The alternative, named as a deliberate scope decision, not a default:** a
  ...
  _more_ convenient in the diarie-present path and _worse_ for the extraction story —
  ...
  skill-side computation), so it _duplicates_ rather than replaces. Reject by default.
...
live in `.ledger/`, the synergy/vendor _registries_ in `.claude/` become the CONFIG
that the `ledger` tool reads to know _which_ relationships exist — exactly the
...
mistake. Narrative that is meant to be read _should_ be visible at the repo root. If
...
2. Build the read side (`ledger review` + the `--json` reader) _before_ moving data,
...
3. Reproduce the privacy invariant structurally (gitignored private subtree or
   filtered-frontmatter + gitignore glob) and prove a public consumer _cannot_ read a
   private entry.
...
1. Add the _optional_ `scope`/`files` field to `diarie/lib/schema.js` (store-only,
...
* **Preserving swarm's real capabilities** (file-disjoint partition + two-reviewer
  ...
  runs depend on them.
* **Privacy invariant reproduction** in `.ledger/` must be _structural_ (a filesystem
