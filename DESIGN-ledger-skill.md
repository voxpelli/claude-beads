...
> **Lead motif** _(every proposal in this document defers to this sentence; a
> mode or a boundary that doesn't serve it gets cut or deferred):_
>
...
this doc governs the _skill shape_, that one governs the _file layout_. The two
...
* **`skills/synergy-tracker/SKILL.md`** — cross-project patterns, divergences,
  ...
  `## Cross-Project Synergy` section of sibling _relationship_ notes.
  **5 workflows.**
...
* **`skills/sibling-sync/SKILL.md`** — bilateral reconciliation of `SYNERGY-*.md`
  and `UPSTREAM-*.md` files _between_ sibling repos (reciprocal gaps, stale
  alignment, status drift, cross-side "shipped" detection). **4 workflows.**

These are not four domains. They are four _verbs_ over two _objects_. The tell
...
* **The project-tempo classifier** — the exact command
  ...
  Guidelines _and_ `synergy-tracker` Guidelines _and_ `sibling-sync` "Project
...
* **The Basic-Memory "division of labor" paragraph** ("these three sections
  ...
  workflow 5 (Promote to Basic Memory) _and_ Guidelines, and `sibling-sync`
...
* **The staleness thresholds** (3 months for an entry; 8 sprints ≈ two
  ...
  already flags the hazard: _"When the canonical thresholds change, this skill
  must be updated to match — the validate-plugin convention check (`vp-beads-9we`)
  catches bare workflow refs but not threshold drift."_
...
_one_ object — a frontend surface. `ledger` has **two** objects (an upstream
dependency, a sibling project), and not every verb applies to both. So `ledger`
is a _sparse 2-D matrix_, where impeccable is a 1-D list. That extra dimension
...
compares a _shared third-party dependency_ as tracked by two sibling repos). Do
...
* **Refresh** — `pull` (upstream subtree), `reconcile` (peer↔peer). _These are
  the two verbs that used to be called "sync" — see the naming caution._
...
convention. It lives (canonically) in the project `CLAUDE.md` and is _cited_ —
...
* **Refresh** — what vp-knowledge's `tag-sync` and `nudge-sync` do: rebuild a
  _derived cache_ (`~/.claude/references/raindrop-tags.md`,
...
Naming it **`ledger`** names the _store/domain_ (the project's book of external
...
**separate skill over separate files**, deliberately _outside_ the tracker.
`diarie` — the current flat-YAML tracker — inherits the same stance: it tracks
_this_ project's work, and knows nothing of siblings or upstreams. `ledger`
...
1. **Skill-only, or a diarie-style CLI + skills combo? — DECIDED: a combo, but
   ...
   staleness all _compute_ over the markdown (parse entries, match titles across
   ...
   **the ledger's value is workflow discipline** (consistently capturing,
   ...
   inline. So the skill ships first and the CLI is extracted _from proven usage_ —
   ...
   is the only writer). The skill's logic _becomes_ the CLI's core; the
...
3. **Does `review --compare` (synergy workflow 3) belong under `review` or
   `reconcile`?** It reads a sibling's _code and conventions_ and proposes new
   entries to `log` here — distinct from `reconcile`, which diffs the _ledger
   files_ bilaterally. Kept under `review` above, but it straddles; the build may
...
* **Broad trigger surface — the real price of the merge.** The four skills
  ...
  absorbs this because its \~20 verbs share _one_ object — `ledger`'s two objects
...
* **Every cross-reference changes.** The `"workflow N (Name)"` convention and
  ...
  `git subtree` from vendor-sync _and_ the BM edit tools from the promoters _and_
