 ...
 *
 * vp-beads ships `diarie` inside a Claude Code plugin, and the plugin's own `.diarie/` is
 * COMMITTED — so a marketplace install copies vp-beads' backlog into every consumer's
 * plugin cache. The CLI resolves a store by walking up from cwd. Run it with a cwd anywhere
 * inside that cache and the walk-up succeeds, on the wrong store, and hands a stranger our
 * 28 tasks as their own. Exit 0. No warning. A plausible, confident, entirely wrong backlog
 * — which is worse than an error, and is the exact failure this tracker was rebuilt to end.
 *
 * `--root` prevents it, and the hooks always pass `--root`. But the skills are prose, and
 * prose forgets: an audit found 71 documented invocations, none of which passed it. A
 * defense that depends on every future sentence remembering is not a defense.
 *
 ...
 * backlog (developing vp-beads itself), say so.
 *
 * @param {string} dir
 * @returns {void}
 * @throws {NoStoreError}
...
export async function loadTasks (root, warn = () => {}) {
  ...
  for (const file of names) {
    ...
    // were perfectly fine.
    //
    // And exit 1 is the code `session-start.sh` reads as ENOSTORE and blanks the payload for — so the
    // hook silently forgot a live claim, on the very path this was all supposed to fix. The founding
    // defect, through a fourth door.
    //
    ...
    for (const t of /** @type {unknown[]} */ (list ?? [])) {
      ...
      //                                                    vp-beads-epc, re-armed, in silence,
      //                                                    by the very guard meant to be safe.
