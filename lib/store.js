  ...
  for (const file of names) {
    ...
    for (const t of list ?? []) {
      ...
      all.push({
        ...
        // `parent` is globalized for the SAME reason `deps` is: the ids this loader hands
        // out are `slug/id`, so a raw `parent: T-0` could never match any of them.
        //
        // It was missing here for the tracker's whole life and broke nothing — because
        // nothing read `parent`. It was a trap for the first reader who did. The container
        // rule (`vp-beads-epc`) was that reader: it would have matched zero children for
        // every epic, excluded none, and gone green. The store hands out ONE id-space;
        // a half-globalized task is a bug waiting for someone to trust it.
