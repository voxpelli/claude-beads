 ...
 * to apply again.
 *
 * THE ONE PLACE THIS RULE LIVES. It used to exist twice — `store.js` had it as
 * `nsId`, `validate.js` had a byte-identical private `glob` — and the copies drifted:
 * `store.js` applied it to `id` and `deps` but forgot `parent`, so `loadTasks` handed
 * out a half-globalized task whose `parent` (`T-0`) could never match any `id`
 * (`slug/T-0`). Every parent lookup silently found nothing. `validate.js` was spared
 * only because its copy was applied to `parent` too.
 *
 * Two implementations of one id rule is what let one of them be incomplete. Keep it
 * here, keep it single, and import it.
 *
