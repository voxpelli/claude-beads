...
import {
  ID_RE, isNil, nsId, RATCHET_TYPES, REQUIRED_FIELDS, VALID_PRIORITIES, VALID_STATUSES, VALID_TYPES,
} from './schema.js'
...
export function lintTasks (files) {
  ...
  for (const [gid, { slug, t }] of all) {
    ...
    if (!isNil(t.parent)) {
      ...
      // A self-parent makes a task its own open child, so the ready-walk would
      // block it on itself, forever, with no cycle to point at. Cheaper to refuse.
    ...
    if (t.status === 'in_progress') {
      for (const d of Array.isArray(t.deps) ? t.deps : []) {
        ...
        if (dep && (dep.status === 'pending' || dep.status === 'in_progress')) {
          warn(slug, `task ${t.id}: in_progress but dep ${nsId(d, slug)} is ${dep.status} (claimed before blockers resolved)`)
        }
      }
    }
