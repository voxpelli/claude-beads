...
export const ready = {
  ...
  async run (argv, meta, { parentName }) {
    ...
    if (ambiguous) {
      const containers = result.blocked.filter(t => t.children?.length).length
      const deps = result.blocked.length - containers
      const what = [deps && `${deps} waiting on deps`, containers && `${containers} epic(s) in flight`].filter(Boolean).join(', ')
      warn(`0 ready, ${result.blocked.length} blocked (${what}) — everything is claimed, or the graph has a cycle. Run \`${parentName} validate\`.`)
    }
      ...
      const lines = opts.blocked
        ...
        ? result.blocked.map(t => {
          const why = [
            ...
            // A blocked row can ALSO be broken. Do not let a live blocker hide a dead one.
    ...
    const brokenButBlocked = result.blocked.some(t => t.attention?.length)
    if (opts.strict && (result.needsAttention.length || ambiguous || brokenButBlocked)) {
