...
The two are **different time horizons, not rivals**: `diarie` is *durable* (the store IS the
repo — it ships on `git push`); the built-in todo list is *ephemeral* (it dies with the
...
execution visible; it is not a place work is *stored*.
...
home" rule, *without* also forbidding the harmless, useful scratchpad.

This is the `substrate-not-opinion` tenet applied: name the invariant that must hold (every
commitment has a durable home) and enforce *that*, rather than banning a whole tool because one
...
- **Keep the blanket ban.** Rejected: it re-teaches the very anti-pattern a cutover is meant to
  end. A migration that retargets the *commands* (`bd` → `diarie`) but leaves the ban in place
...
Codified **2026-07-11** in `6dd4795` (*"the ban on the agent's own task list was ours, and we
renewed it"*): the ban was removed from `CLAUDE.md` and the seam rule written into the
...
- **`migrate-tracker`** *offers* this seam to a bd-era target project that still carries a
