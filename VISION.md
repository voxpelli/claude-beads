...
next decade of software — _how should a developer work with agents?_ There are two honest
poles:
...
| Concurrency  | A _fleet_: a Mayor slinging work to 20–30 agents                                | _Choreography_: file-disjoint waves of ephemeral agents, one human conductor                                                  |
...
| For          | Teams running continuous autonomous agents, where coordination _is_ the product | Solo + small-team sovereign builders who want agents to amplify **one mind** and refuse a daemon between them and their files |
...
pole is the answer for the sovereign solo builder. We are not better; we are the _other
answer_, and we own it deliberately.
...
The stance above is the motif's _why_. When a feature is proposed, the question is: _does it
belong to the calm-sovereign pole, or the Gas Town pole?_ The two-poles table is the
adjudicator.
...
* **No SQL / binary state store.** Plain text + git + Basic Memory. (This is _why_ we are
...
* **No host colonization.** We never inject directives that tell Claude Code to stand down
  its own memory/task primitives. We live _alongside_ the platform, not in tension with it.
* **No workflow-prescribing substrate.** The substrate provides primitives; the _skills_
...
The stance is not invented here; it is _recognized_. It rests on a real intellectual canon:
...
* **Convivial tools** — Ivan Illich, _Tools for Conviviality_ (autonomy-preserving,
  human-scale tools vs. tools that dominate their users) — the keystone that names _why_ a
