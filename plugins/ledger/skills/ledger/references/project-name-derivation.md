...
filenames, and inverse-file lookups across sibling projects. See the ledger
mode files that reference this document for their steps.
...
convention" section and `SKILL.md` "File conventions"). Vendor and sibling identities use the same normalization
...
in `SKILL.md` "Registries"). Find the entry pointing back at this
...
This tier honors the `synergy-entry-format.md` "Naming convention" rule that the
registry `name` field is authoritative — the sibling already chose a canonical label
...
unreachable because those modes never read the sibling's manifest — the
sibling's `name` comes from this project's `synergy-registry.json` (which is
itself a tier-3-style source for the sibling subject).
...
`references/synergy-entry-format.md` "Naming convention" section. Apply those
rules to the raw name produced by whichever tier fired:
...
- **YAGNI position on ecosystem expansion**: the algorithm deliberately
  ...
  family, extend tier 3 to probe the relevant manifest. See the open task
