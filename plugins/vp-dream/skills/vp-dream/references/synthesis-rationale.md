...
| **`jl-cmd/claude-dream`** (community plugin)                               | The explicit Audit → Propose → **approve** → Execute → Verify loop, the index-integrity check, dedup/rename/staleness flags                                                                    | Turns the native pass into an _inspectable, approval-gated, manual_ pass. Good hygiene checklist.                                                                      |
...
step is Verify: for every _volatile_ memory, re-confirm against a primary source
...
* **Native autoDream** runs in the background automatically. This skill is the
  _manual, verification-augmented_ trigger. If native autoDream is enabled and
...
`originSessionId`; `jl-cmd/claude-dream` hardcoded a _flat_ `type`, a 200-line
...
these references as _fallback illustration that may be stale_. When they
