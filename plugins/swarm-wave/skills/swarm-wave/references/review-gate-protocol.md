...
A guard is a claim about inputs it has never seen. Whoever wrote it chose the inputs it _was_
...
* The `invalid:` fixtures for a new ast-grep rule were **confirmatory** — they proved the rule
  ...
  ways within minutes (an `exit(2)` in the ELSE arm, one in a nested closure, one merely _wrapped_
...
> human asking _"did you run the gate?"_ — so the honest mitigation is to make the question easy to
