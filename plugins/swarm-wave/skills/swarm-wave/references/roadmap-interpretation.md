...
in _its own_ idiom and vocabulary. It never reformats the file, never imposes a
...
A milestone plan may _contain_ wave subsections (Repo A's `### Wave N` live
...
A wave or item that is already done must be excluded. Recognize _any_ of these
...
**Default rule:** a wave with a recognized _done_ marker is excluded; a wave with
a recognized _active_ marker (`(NEXT)`, no done-marker) is included; a wave whose
